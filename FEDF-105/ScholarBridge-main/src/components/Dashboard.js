...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    if (userId) {
      // Firebase-backed parts (only when signed into Firebase)
      ...
      // No Firebase user; don't hang waiting for snapshots
      setLoading(false);
    }
  }, [user]);

  // Always fetch backend-powered data (does not require Firebase auth)
  useEffect(() => {
    let mounted = true;
    (async () => {
      try {
        ...
        setCertificates((certsData || []).length);
      } catch (e) {
        // Silently ignore in UI; could add toast/log if desired
      }
    })();
  ...
  const countAcademic = useMemo(() => activities.filter((a) => a.type === "academic").length, [activities]);
  const countCoCurricular = useMemo(() => activities.filter((a) => a.type === "co-curricular").length, [activities]);
